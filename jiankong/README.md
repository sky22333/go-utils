...
- ⚠️ **智能告警**: CPU/内存使用率超过阈值自动发送告警，支持自定义告警间隔避免重复通知
- 🌍 **地理位置**: 自动获取服务器 IP 和地理位置信息
- 🤖 **交互式界面**: 内置按钮支持实时查询
- 🔧 **灵活配置**: 支持环境变量和 JSON 配置文件
- 🔒 **权限控制**: 只有管理员可以修改配置，确保系统安全
...
| 配置项 | 环境变量 | 默认值 | 说明 |
|--------|----------|--------|------|
| 报告时间 | `REPORT_TIME` | `15:00` | 每日报告时间（24小时制），可通过机器人按钮配置 |
| 自定义消息 | `CUSTOM_MESSAGE` | `🖥️ 服务器状态报告` | 自定义提示信息，可通过机器人按钮配置 |
| CPU 阈值 | `CPU_THRESHOLD` | `80` | CPU 告警阈值（百分比），可通过机器人按钮配置 |
| 内存阈值 | `MEM_THRESHOLD` | `80` | 内存告警阈值（百分比），可通过机器人按钮配置 |
| 告警间隔 | `ALERT_INTERVAL` | `10` | 告警通知间隔（分钟），避免重复通知，可通过机器人按钮配置 |

> 💡 **新功能**: 现在可以直接在Telegram机器人中通过按钮交互来配置报告时间、自定义消息和告警阈值，无需重启服务！

## 配置文件说明

项目现在支持两种配置文件：

1. **config.json** - 基础配置文件，包含必需的机器人Token和Chat ID
2. **user_config.json** - 用户配置文件，通过机器人交互自动生成和更新

配置优先级：`机器人交互配置` > `环境变量` > `config.json` > `默认值`
