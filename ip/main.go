...
var (
	...
	logoPath   = flag.String("logo", "", "Logo图片路径，支持PNG/JPEG格式，留空使用默认logo")
...
func loadLogo() {
	if *logoPath != "" {
		...
		logoImage = resizeLogo(img, 64, 64)
...
func generateSVG(ip, ua, loc, now string) string {
	var logoBase64 string
	if logoImage != nil {
		var buf bytes.Buffer
		if err := png.Encode(&buf, logoImage); err == nil {
...
func generatePNG(ip, ua, loc, now string) []byte {
	dc := contextPool.Get().(*gg.Context)
	defer contextPool.Put(dc)
...
func ipImageHandler(c *gin.Context) {
	ip := getClientIP(c)
	...
	if isSVG {
		...
		c.String(http.StatusOK, generateSVG(ip, ua, loc, now))
	} else {
		pngData := generatePNG(ip, ua, loc, now)
		if pngData == nil {
...
func main() {
	...
	api := r.Group("/api")
	{
		api.GET("/ip", ipImageHandler)
		api.GET("/ip.png", ipImageHandler)
		api.GET("/ip.svg", ipImageHandler)
	}
	
	r.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{
			"message": "github.com/sky22333/go-utils",
